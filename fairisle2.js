...
var nx = W/dx/d;
var ny = Math.round(H/dy/d+1);
	
var punchcard = [];
punchcard=PC_gingham;

var punch = [0,1,0,1,0,1,0,1,1,1,0,0];
...
// 		}	
 		...
 		ctx.beginPath();
		//ctx.transform(dx*d,0,0, 100*d,0,0);
		for (i=0; i<nx; i++){
			for(j=ny-1; j>=0; j--){
...
// 	}
	...
	ctx.beginPath();
	ctx.transform(dx*d,0,0, 100*d,parseInt((xpos)/W*nx)*dx*d,parseInt((ypos)/H*(ny-1))*d*dy);
	ctx.moveTo(0.15,0);
...
// 	ctx.lineTo(0.15,0);
// 	ctx.moveTo(0.85,0);
// 	ctx.bezierCurveTo(1.1,0.2, 0.95,0.4, 0.6,1.02);
// 	ctx.bezierCurveTo(0.52,0.75, 0.4,0.75, 0.8,0.1);
// 	ctx.lineTo(0.85,0);
	ctx.moveTo(0.15,0);
	ctx.lineTo(0.4,1.02);
	ctx.lineTo(0.2,0.1);
